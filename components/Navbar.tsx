...
export default function Navbar({ user }: NavbarProps): React.ReactElement {
    ...
    return (
      <nav className="bg-black-background border-b! border-gray-text! flex flex-row items-center justify-between px-6 py-3 md:py-5 relative">
        ...
        {isUserClicked && (
          <div className="absolute right-0 bg-color-white text-black top-20 w-fit h-fit z-50 rounded-md p-4 shadow-lg">
            <ul className="flex flex-col gap-2">
              <li>
                <Link
                  href="/dashboard"
                  className="font-bold hover:text-red-brand transition-colors"
                >
                  Dashboard
                </Link>
              </li>
